...
3. Security & Identity (JWT)
    ...
    Flow: The frontend authenticates via a guest login, receives a bearer token, and stores it in LocalStorage. The backend extracts the User ID from the token signature, ensuring requests cannot be spoofed.
    
4. High-Performance Data Seeding
    Problem: Initializing 5,000 seats using individual save() calls took >30 seconds.
    Solution: Implemented Batch Processing (repository.saveAll) to group inserts.
